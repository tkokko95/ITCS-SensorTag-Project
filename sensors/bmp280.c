...
void bmp280_get_data(I2C_Handle *i2c, double *pres_ptr, double *temp_ptr) {
    ...
    if (I2C_transfer(*i2c, &i2cTransaction)) {
		...
		//Ilmanpaineen ja lämpötilan erottelu puskurista:
		int32_t pres = 0x00000000;
		int32_t temp = 0x00000000;
		
		pres = pres | (rxBuffer[2] & 0b11110000) | rxBuffer[1] << 4 | rxBuffer[0] << 12;
		temp = temp | rxBuffer[5] & 0b11110000 | rxBuffer[4] << 4 | rxBuffer[3] << 12;
		
		//Lämpötilan muunnos, ei olla jenkkejä joten muutetaan samalla celsiusasteiksi
		*temp_ptr = (bmp280_convert_temp(temp) - 32) / 1.8;
		
		//Ilmanpaineen muunnos ensin pascaleiksi ja sitten hehtopascaleiksi
		*pres_ptr = bmp280_convert_pres(pres) / 100;
