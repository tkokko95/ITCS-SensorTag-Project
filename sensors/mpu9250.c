...
void mpu9250_get_data(I2C_Handle *i2c, float *mpu_buffer) {

	uint8_t rawData[6];
	int16_t data[3]; 
	
	//Gyroa ei tarvita
    readByte( ACCEL_XOUT_H, 6, &rawData[0]);  
	data[0] = (int16_t)(((int16_t)rawData[0] << 8) | rawData[1]) ;  
	...
	*mpu_buffer = (float)data[0]*aRes - accelBias[0];
	*(mpu_buffer + 1) = (float)data[1]*aRes - accelBias[1];
	*(mpu_buffer + 2) = (float)data[2]*aRes - accelBias[2];
